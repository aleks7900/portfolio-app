...
export default function UsersPage() {
    ...
    const onExportCSV = async () => {
        try {
            const all = await getJSON<UsersListResponse>(`${API_BASE}/users/list?${qs({
                ...params,
                page: 0,
                size: 10000
            })}`);
            ...
            a.href = url;
            a.download = `users_${from || "from"}_${to || "to"}.csv`;
        ...
        <div className="mx-auto w-full max-w-7xl p-4 mt-20 md:p-6 space-y-6">
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <Input value={q} onChange={(e) => setQ(e.target.value)}
                           placeholder={tf("users_searchPlaceholder", "Поиск по userId/email")} className="w-56"/>
                    <Input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="w-36"
                           title="from"/>
                    <Input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="w-36" title="to"/>
                    <Select value={String(size)} onValueChange={(v) => setSize(Number(v))}>
                        <SelectTrigger className="w-28 dark:text-black"><SelectValue
                            placeholder="20 / стр."/></SelectTrigger>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                {loading && !summary ? (
                    ...
                    <>
                        <Card className="dark:bg-zinc-800"><CardHeader className="pb-2"><CardTitle
                            className="text-sm text-muted-foreground">{tf("users_total", "Всего пользователей")}</CardTitle></CardHeader><CardContent
                            className="pt-0">
                            <div className="text-2xl font-bold">{numberFmt.format(summary.totalUsers)}</div>
                        </CardContent></Card>
                        <Card className="dark:bg-zinc-800"><CardHeader className="pb-2"><CardTitle
                            className="text-sm text-muted-foreground">{tf("users_active7d", "Активные 7д")}</CardTitle></CardHeader><CardContent
                            className="pt-0">
                            <div className="text-2xl font-bold">{numberFmt.format(summary.active7d)}</div>
                        </CardContent></Card>
                        <Card className="dark:bg-zinc-800"><CardHeader className="pb-2"><CardTitle
                            className="text-sm text-muted-foreground">{tf("users_active30d", "Активные 30д")}</CardTitle></CardHeader><CardContent
                            className="pt-0">
                            <div className="text-2xl font-bold">{numberFmt.format(summary.active30d)}</div>
                        </CardContent></Card>
                        <Card className="dark:bg-zinc-800"><CardHeader className="pb-2"><CardTitle
                            className="text-sm text-muted-foreground">{tf("users_newToday", "Новые сегодня")}</CardTitle></CardHeader><CardContent
                            className="pt-0">
                            <div className="text-2xl font-bold">{numberFmt.format(summary.newToday)}</div>
                        </CardContent></Card>
                        <Card className="dark:bg-zinc-800"><CardHeader className="pb-2"><CardTitle
                            className="text-sm text-muted-foreground">{tf("users_avgDuration", "Средняя длительность")}</CardTitle></CardHeader><CardContent
                            className="pt-0">
                            <div className="text-2xl font-bold">{durFmt(summary.avgSessionDurationSec)}</div>
                        </CardContent></Card>
                    </>
                ) : error ? (
                    <div className="col-span-full">
                        <Alert
                            variant="destructive"><AlertDescription>{tf("users_error", "Ошибка загрузки")}: {error}</AlertDescription></Alert>
                    </div>
                ) : null}
            </div>
            ...
            <Card className="overflow-hidden dark:bg-zinc-800">
                <CardHeader className="pb-2"><CardTitle
                    className="text-base text-muted-foreground">{tf("users_tableTitle", "Список пользователей")}</CardTitle></CardHeader>
                <CardContent>
                    {loading && !list ? (
                        ...
                        <>
                            <table className="w-full text-sm">
                                ...
                                <tr className="text-left text-muted-foreground border-b">
                                    ...
                                    <th className="py-2 px-2 cursor-pointer select-none"
                                        onClick={() => toggleSort("lastSeen")}>
                                        {tf("users_lastSeen", "Последний визит")} <ArrowUpDown
                                        className="inline h-4 w-4 ml-1"/>
                                    </th>
                                    <th className="py-2 px-2 cursor-pointer select-none text-right"
                                    ...
                                    <th className="py-2 px-2 cursor-pointer select-none text-right"
                                        onClick={() => toggleSort("avgSessionDurationSec")}>
                                        {tf("users_avgDur", "Средн. длит.")} <ArrowUpDown
                                        className="inline h-4 w-4 ml-1"/>
                                    </th>
                                    <th className="py-2 px-2 cursor-pointer select-none text-right"
                            ...
                            <div className="flex items-center justify-between mt-4">
                                ...
                                <div className="flex items-center gap-2">
                                    <Button variant="outline" size="sm" onClick={() => {
                                        ...
                                        loadAll();
                                    }} disabled={!canPrev} className="dark:text-black">
                                        <ChevronLeft className="h-4 w-4"/>{tf("users_prev", "Назад")}
                                    </Button>
                                    <div
                                        className="text-sm">{list.page + 1} / {Math.max(1, Math.ceil(list.total / list.size))}</div>
                                    <Button variant="outline" size="sm" onClick={() => {
                                        setPage(p => p + 1);
                                        loadAll();
                                    }} disabled={!canNext} className="dark:text-black">
