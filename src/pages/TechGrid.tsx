    ...
    <section className="relative py-20 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-5">
          {techs.map(({ icon: Icon, label, color, slug }, index) => (
            <motion.div
              key={label}
            ...
            >
              <Link
                to={slug}
                className="group relative flex flex-col items-center justify-center rounded-2xl border border-border/80 bg-card/70 dark:bg-card/50 p-6 shadow-xs hover:shadow-xl hover:border-primary/40 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1.5 cursor-pointer text-center h-full"
              >
                {/* Subtle hover glow accent */}
                <div className="absolute inset-0 rounded-2xl bg-primary/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />

                <div className="mb-3 transition-transform duration-300 group-hover:scale-110">
                  <Icon className={`w-12 h-12 ${color} transition-colors duration-300`} />
                </div>

                <p className="font-semibold text-xs tracking-wider uppercase text-foreground/80 group-hover:text-foreground transition-colors">
                  {label}
                </p>
              </Link>
            </motion.div>
