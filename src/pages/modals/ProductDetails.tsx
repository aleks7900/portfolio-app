...
import {type Lang, useI18n} from "../../shared/i18n/i18n.tsx";
...
export default function ProductDetails({
    ...
    const toLang = (s?: string): Lang => (s === "ro" ? "ro" : "ru");

    // Текущий язык
    const langStr = i18n?.language?.split("-")[0];
    const lang: Lang = toLang(langStr);             // <- теперь не string, а "ru" | "ro"
    ...
    const localizedName = t(normalizeKey(product.title));
    ...
    const description = seoEntry?.description ?? fallback.description;
    const keywords = seoEntry?.keywords ?? fallback.keywords;
    ...
    // Аккуратная карта доступности для JSON-LD
    const offerAvailability =
        product?.inStock
            ? "http://schema.org/InStock"
            : product?.availability === "ORDER_ON_DEMAND"
    ...
    return (
        <AnimatePresence>
            {open && (
                <>
                    {/* ✅ SEO для страницы товара — берём контент по ключам из seo_*.json */}
                    <SEO
                        title={title}
                                ...
                                <div className="mt-4 rounded-2xl border p-4 dark:border-white/10">
                                    {tab === "details" && (
                                        <div className="prose max-w-none text-sm dark:prose-invert">
                                            <p>
                                                {t(product.title.toLowerCase())} — {product.brand}. Отличный выбор.
                                                Цена:
                                    ...
                                    {tab === "reviews" && (
                                        <div className="space-y-3">
                                            {product.reviews?.length ? (
                                                ...
                                                <div className="text-gray-500 dark:text-gray-400">Отзывов пока
                                                    нет.</div>
                                            )}
                                        </div>
                                    )}
