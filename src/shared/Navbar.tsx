...
// Закрыть по Escape
    ...
    return (
        <div className="relative" ref={containerRef}>
            ...
            {open && (
                <div role="menu" className="absolute right-0 mt-2 w-[680px] rounded-2xl border bg-white p-4 shadow-lg dark:bg-black dark:border-white/10">
                    <div className="grid grid-cols-3 gap-4">
                        {CATS.map(cat => (
                            <div key={cat.key}>
                                <div className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-200">{t(cat.labelKey)}</div>
                                <div className="grid">
                                    {cat.children?.map(sub => (
                                        <button
                                            key={sub.key}
                                            role="menuitem"
                                            onClick={() => { navigate(`/catalog/${cat.key}/${sub.key}`); setOpen(false); }}
